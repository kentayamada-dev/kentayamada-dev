...
import type { ContactSchemaType, FormStateType } from './types';
...
const createPost = async (_prevState: FormStateType, formData: FormData): Promise<FormStateType> => {
  const rawData = Object.fromEntries(formData);
  const parsed = contactSchema.safeParse(rawData);

  if (!parsed.success) {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
    return { data: rawData as ContactSchemaType, errors: parsed.error.formErrors };
  }
